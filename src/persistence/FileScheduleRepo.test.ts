...
test("list of days off returns only dates within specified range", () => {
    const repo = new FileScheduleRepo(testFile);
    const entries = [
        { dateISO: "2023-10-10", reason: DayOffReason.HOLIDAY, scope: 'ALL_STUDENTS' as const },
        { dateISO: "2023-11-15", reason: DayOffReason.PROF_DEV, scope: 'ALL_STUDENTS' as const },
        { dateISO: "2023-12-20", reason: DayOffReason.HOLIDAY, scope: 'ALL_STUDENTS' as const }
      ];
